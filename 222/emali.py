...
try:
    users = [x.strip() for x in open(sys.argv[2])]
    words = [x.strip() for x in open(sys.argv[3])]
except(IOError):
...
if __name__ == '__main__':
    ...
    if len(success) >=1:
	filename = open('emailresult.txt','a')
        for ret in success:
            print "\n\n[+] Login successful:",ret[0], ret[1]
            print "\t[+] Mail:",ret[2],"emails"
            print "\t[+] Size:",ret[3],"bytes\n"
	    strings = str(ret[0]+ret[1]+ret[2]+ret[3]+'\r')
	    filenames.write(strings)
    print "\n[-] Done"
